...
router.get('/sections', (req, res) => {
    ...
    console.log('Received Query Params:', req.query);  // Log the query parameters to verify

    if (!subject || !fiscalYear) {
        return res.status(400).json({ error: 'Subject and fiscal year are required' });
    }

    // Update this mapping logic to handle fiscalYear '1' and '2' correctly
    const fiscalYearId = fiscalYear === '1' ? 1 : (fiscalYear === '2' ? 2 : null);

    console.log('Mapped Fiscal Year:', fiscalYearId); // Log the fiscal year ID
...
AND sch.fiscal_year_id = ?
    ...
    db.query(query, [subject, fiscalYearId], (error, results) => {
        ...
        console.log('Sections fetched:', results);  // Log to confirm fetched sections
...
router.get('/students', (req, res) => {
    const { subject, section, fiscalYear } = req.query;  // Removed session from query params

    if (!subject || !section || !fiscalYear) {
        return res.status(400).json({ error: 'Subject, Section, and Fiscal Year are required' });
    }
    ...
    db.query(query, [subject, section, fiscalYearId], (error, studentResults) => {
        if (error) {
        ...
        res.json(studentResults);  // Return student data to the frontend
    });
});
...
router.post('/insert-time-slot', (req, res) => {
    ...
    console.log(`Received request to insert or update a time slot with start time: ${startTime}, end time: ${endTime}`);

    // Step 3: Check if the time slot already exists
    const checkTimeSlotQuery = `
        SELECT * FROM time_slots WHERE time_start = ? AND time_end = ?;
    `;
    
    db.query(checkTimeSlotQuery, [startTime, endTime], (err, result) => {
        if (err) {
            console.error("Error checking for existing time slot: ", err);
            return res.status(500).json({ error: "Error checking time slot." });
        }

        // If time slot exists, update it
        if (result.length > 0) {
            const updateTimeSlotQuery = `
                ...
                WHERE id = ?;
            `;
            db.query(updateTimeSlotQuery, [startTime, endTime, result[0].id], (err, updateResult) => {
                if (err) {
                ...
                res.status(200).json({ message: "Time slot updated successfully", startTime, endTime });
            });
        } else {
            // If time slot doesn't exist, insert a new one
...
router.get('/get-time-slots', (req, res) => {
    // Log for debugging
    console.log('Fetching time slots from the database.');

    // Query the database to fetch all time slots
    ...
    db.query(query, (err, results) => {
        ...
        // Convert time slots to AM/PM format
        const formattedTimeSlots = results.map(slot => {
            return {
                ...slot,
                time_start: convertToAMPM(slot.time_start),
                time_end: convertToAMPM(slot.time_end)
            };
        });
